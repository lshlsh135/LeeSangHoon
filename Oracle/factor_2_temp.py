...
class factor_3:
    ...
    def factor_3(self):
        ...
        for n in range(col_length): 
            ...
            first_data = first_data[first_data.iloc[:,col_num].notnull()] # 1/PER이 NULL인걸 제외한다. EQUITY 가 NULL인걸 미리 제거하지 않아도 여기서 제거됨.
            first_data = first_data[first_data.iloc[:,col_num2].notnull()]
            first_data = first_data[first_data.iloc[:,col_num3].notnull()]
            ...
            first_data=first_data.assign(rnk1=np.floor(first_data.iloc[:,col_num].rank(method='first')))
            first_data=first_data.assign(rnk2=np.floor(first_data.iloc[:,col_num2].rank(method='first')))
            first_data=first_data.assign(rnk3=np.floor(first_data.iloc[:,col_num3].rank(method='first')))
...
#               
        ...
        return [return_final, return_diff]  # 이렇게 return 하면 list로 받아짐
