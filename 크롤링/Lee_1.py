...
win_data = pd.DataFrame(np.zeros((len(result_drop),7)))
...
for n in range(len(result_drop)):
    ...
    if temp.empty:    # 영업이익이 포함된 row index가 없다면.... 연결 포괄손익계산서가 이상하게 쓰여진거일듯
        ...
        if df.iloc[ebit_row_num_temp,0] > df.iloc[ebit_row_num_temp,2]:
            ...
            win_data.iloc[a,3] = df.iloc[ebit_row_num_temp,0]
            win_data.iloc[a,4] = df.iloc[ebit_row_num_temp,2]
            win_data.iloc[a,5] = (df.iloc[ebit_row_num_temp,0]-df.iloc[ebit_row_num_temp,2])/df.iloc[ebit_row_num_temp,2]
            win_data.iloc[a,6] = result_drop.loc[n,'rpt_nm']
            a = a+1
    else:
        ebit_row_num_temp = temp.iloc[0].name
        if df.iloc[ebit_row_num_temp,0] > df.iloc[ebit_row_num_temp,2]:
...
for p in range(len(win_data)):
    if win_data.iloc[p,5]<0:
        win_data.iloc[p,5] = win_data.iloc[p,5]*(-1)
