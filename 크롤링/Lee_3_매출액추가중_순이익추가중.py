# -*- coding: utf-8 -*-
"""
Created on Wed Sep 13 11:33:13 2017
...
win_data = pd.DataFrame(np.zeros((len(result_drop)*4,19)))
        ...
        if len(df.columns)>3:        
            ...
            if temp.empty:    # 영업이익이 포함된 row index가 없다면.... 연결 포괄손익계산서가 이상하게 쓰여진거일듯
                ...
                if (df.iloc[ebit_row_num_temp,0]!=' ')&(df.iloc[ebit_row_num_temp,2]!=' '):
                    ...
                    win_data.iloc[a,14] = df1
                    win_data.iloc[a,15] = row_name[ebit_row_num_temp]
                    win_data.iloc[a,16] = df.iloc[ebit_row_num_temp,0]
                    win_data.iloc[a,17] = df.iloc[ebit_row_num_temp,2]
                    if df.iloc[ebit_row_num_temp,2]!=0:
                        win_data.iloc[a,18] = (df.iloc[ebit_row_num_temp,0]-df.iloc[ebit_row_num_temp,2])/df.iloc[ebit_row_num_temp,2]
            ...
            if (win_data.iloc[a-1,0]==0)&(win_data.iloc[a-1,0]==0): 
                 win_data.iloc[a-1,0] = result_drop.loc[n,'rcp_dt']
                 win_data.iloc[a-1,1] = result_drop.loc[n,'crp_cd']
                 win_data.iloc[a-1,2] = result_drop.loc[n,'crp_nm']
                 win_data.iloc[a-1,3] = result_drop.loc[n,'rpt_nm']
